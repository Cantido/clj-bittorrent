...
(deftest read-file-test
  (let [result (m/read metainfo-file)]
    (is (= 1599
           (m/expected-piece-count result)))

    (is (= "https://torrents.linuxmint.com/announce.php"
           (:announce result)))

    (is (= "Transmission/2.84 (14307)"
           (:created-by result)))
    ...
    (let [info (:info result)]
      ...
      (is (= "linuxmint-18.2-cinnamon-64bit.iso"
             (:name info)))
      ...
      (is (= 31980
             (mapreduce count + (:pieces info))))

      (is (= true
             (every? b/ubyte? (apply concat (:pieces info))))))))
    ...
    (let [info (:info result)]
      ...
      (let [files (:files info)]
        ...
        (let [first-file (first files)]
          ...
          (is (= "aae98423363bcafc54b03289f222612b9e0004b0"
                 (get first-file "sha1")))

          (is (= "54cf30db294b07c6a26293c0a8aa8aee"
                 (get first-file "md5")))

          (is (= "44504df6"
                 (get first-file "crc32"))))))))
